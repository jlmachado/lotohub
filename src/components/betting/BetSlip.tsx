/**
 * @fileOverview Carrinho de Apostas Profissional (Sportsbook Standard).
 */
...
import { X, Trash2, ReceiptText, TrendingUp, Wallet, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[100] w-[calc(100%-32px)] max-w-sm animate-in slide-in-from-right-4 duration-500">
      <Card className="shadow-2xl border-primary/30 bg-[#0f172a] text-white overflow-hidden ring-1 ring-white/10">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-sm font-black uppercase italic tracking-tight">
                {isMultiple ? `Múltipla (${betSlip.length})` : 'Simples'}
              </CardTitle>
              <p className="text-[9px] font-bold text-primary/70 uppercase tracking-widest mt-0.5">Bilhete de Apostas</p>
            </div>
          </div>
          <Button 
            variant="ghost" 
        ...
        {/* Itens do Bilhete */}
        <CardContent className="p-3 space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar bg-slate-900/30">
          {betSlip.map((bet) => (
            <BetSlipItem 
              key={bet.id} 
              item={bet} 
              onRemove={() => removeBetFromSlip(bet.id)} 
            />
          ))}
        </CardContent>
        ...
        <CardFooter className="p-4 flex-col gap-4 border-t border-white/10 bg-[#020617]">
          <div className="w-full space-y-4">
            <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl border border-white/5 shadow-inner">
              ...
              <span className="font-black text-primary text-xl italic tracking-tighter tabular-nums">
                {totalOdds.toFixed(2)}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                <span className="text-[9px] font-black text-green-500/70 uppercase px-1">Ganhos Possíveis</span>
          ...
          >
            {stake > balance ? 'Saldo Insuficiente' : 'Finalizar Bilhete'}
