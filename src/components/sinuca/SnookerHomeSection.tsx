 ...
 * Refatorado para exibir múltiplos jogos ao vivo e próximos jogos baseados em tempo real.
 */
...
import { getSnookerMarketState, isSnookerVisibleOnHome, resolveSnookerChannelStatus } from '@/utils/snooker-rules';
...
export const SnookerHomeSection = () => {
  ...
  // Atualiza o relógio a cada minuto para manter o status de visibilidade preciso
  useEffect(() => {
  ...
  const activeGames = useMemo(() => {
    return (snookerChannels || [])
      .filter(c => isSnookerVisibleOnHome(c, now))
      .map(c => {
        const resolvedStatus = resolveSnookerChannelStatus(c, now);
        return {
          ...c,
          resolvedStatus,
          market: getSnookerMarketState(c, now)
        };
      })
      .sort((a, b) => {
        // Live primeiro
        ...
        if (a.resolvedStatus !== 'live' && b.resolvedStatus === 'live') return 1;
        // Imminent depois
        if (a.resolvedStatus === 'imminent' && b.resolvedStatus !== 'imminent') return -1;
        if (a.resolvedStatus !== 'imminent' && b.resolvedStatus === 'imminent') return 1;
        // Por fim, prioridade configurada
  ...
  return (
    <section className="px-4 space-y-4 mb-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-muted-foreground uppercase font-bold tracking-widest">Eventos reais sincronizados agora</p>
          </div>
        </div>
        <Button 
          variant="link" 
          onClick={() => router.push('/sinuca/ao-vivo')} 
          className="text-[10px] font-black uppercase text-primary italic gap-1"
        >
          Ver Todos ({activeGames.length}) <ArrowRight size={12} />
        </Button>
      </div>
        ...
        {activeGames.map(game => (
          <Card 
            ...
            className={cn(
              ...
              game.resolvedStatus === 'live' && "ring-1 ring-red-500/20"
            )}
            onClick={() => router.push('/sinuca/ao-vivo')}
          >
            <div className={cn(
              "p-2 px-3 border-b flex justify-between items-center",
              game.resolvedStatus === 'live' ? "bg-red-600/10 border-red-500/10" : "bg-white/5 border-white/5"
            )}>
              <div className="flex items-center gap-2">
                {game.resolvedStatus === 'live' ? (
                  <Badge className="bg-red-600 text-white text-[8px] font-black h-4 px-1.5 animate-pulse">AO VIVO</Badge>
                ) : (
                  <div className="flex items-center gap-1 text-slate-500">
                    <Calendar size={10} />
                    <span className="text-[8px] font-black uppercase">
                      {new Date(game.scheduledAt || game.createdAt).toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit'})}
                    </span>
                  </div>
                )}
              </div>
              <Badge variant="outline" className={cn("text-[8px] h-4 uppercase font-black border-white/10", game.market.color)}>
                {game.market.isBettable ? 'MERCADO ABERTO' : game.market.label}
              </Badge>
            </div>
            ...
            <CardContent className="p-4 space-y-4">
              <div className="flex justify-between items-center text-center gap-2">
                ...
                <div className="flex flex-col items-center">
                  <div className="text-xl font-black italic text-primary tabular-nums tracking-tighter">
                    {game.resolvedStatus === 'live' ? `${game.scoreA} - ${game.scoreB}` : 'vs'}
                  </div>
              ...
              <div className="flex items-center justify-center gap-2 pt-2">
                ...
                {game.market.isBettable && (
                  <Button size="sm" className="h-8 flex-1 bg-primary text-black font-black uppercase italic text-[10px] rounded-lg lux-shine shadow-lg shadow-primary/10">
