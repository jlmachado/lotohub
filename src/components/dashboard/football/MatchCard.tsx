...
import { Radio, Calendar, Lock, ShieldCheck } from 'lucide-react';
...
export function MatchCard({ match, onSelectOdd, isSelected, disabled }: MatchCardProps) {
  const kickoffDate = new Date(match.kickoff || Date.now());
  const kickoffTime = kickoffDate.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
  const kickoffDay = kickoffDate.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
        ...
        <div className="flex items-center gap-2">
          {match.isLive ? (
            ...
            <>
              ...
              <span className="text-[8px] font-black uppercase text-muted-foreground">{kickoffDay} {kickoffTime}</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-2">
        ...
        {(isSuspended || isClosed) && (
          <div className="text-center py-2 bg-black/40 rounded-xl border border-dashed border-white/10 animate-in fade-in">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest italic">
              {isClosed ? 'MERCADO ENCERRADO' : 'MERCADO SUSPENSO'}
            </p>
          </div>
        )}
      </CardContent>
