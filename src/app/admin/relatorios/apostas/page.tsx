...
export default function RelatorioApostasPage() {
  ...
  const handleExport = () => {
    downloadCSV(
      `relatorio_apostas_${context?.subdomain || 'global'}_${new Date().toISOString().split('T')[0]}.csv`,
      filtered.map(r => ({
      ...
      ['Data', 'ID', 'Terminal', 'Usuário', 'Módulo', 'Descrição', 'Valor']
    );
  };

  const handleClear = () => {
    if (context) clearReportFilters(context, "apostas");
    handleDefaultRange();
    setSearchTerm('');
    setCurrentPage(1);
  };
