 ...
 * Gerencia o estado global, persistência local e automação de Sinuca/Futebol.
 */
...
import { SnookerSyncService, SyncSummary } from '@/services/snooker-sync-service';
...
export interface BingoWinner { category: string; userId: string; userName: string; terminalId: string; winAmount: number; winningNumbers: number[]; wonAt: string; type: 'USER_WIN' | 'BOT_WIN'; }
export interface BingoDraw { id: string; drawNumber: number; status: 'scheduled' | 'waiting' | 'live' | 'finished' | 'cancelled'; drawnNumbers: number[]; winnersFound: { quadra?: BingoWinner; kina?: BingoWinner; keno?: BingoWinner; }; totalTickets: number; totalRevenue: number; payoutTotal: number; bancaId: string; scheduledAt: string; startedAt?: string; finishedAt?: string; ticketPrice: number; prizeRules: { quadra: number; kina: number; keno: number; }; housePercent: number; }
...
export interface SnookerChannel { 
  ...
  isArchived?: boolean; 
  bettingAvailability?: 'disabled' | 'prelive' | 'live_only' | 'all';
...
export interface SnookerChatMessage { id: string; channelId: string; userId: string; userName: string; text: string; createdAt: string; role: 'user' | 'admin'; deleted?: boolean; }
export interface SnookerScoreboard { matchTitle: string; playerA: { name: string; score: number }; playerB: { name: string; score: number }; scoreA: number; scoreB: number; statusText: string; frame: number; round: { number: number; endsAt?: string }; }

// --- CONTEXT TYPE ---
interface AppContextType {
  user: any; allUsers: any[]; isLoading: boolean; balance: number; bonus: number; terminal: string; activeBancaId: string; ledger: any[]; banners: Banner[]; popups: Popup[]; news: NewsMessage[]; apostas: Aposta[]; postedResults: any[]; jdbResults: JDBNormalizedResult[]; jdbSyncLogs: SyncLogEntry[]; jdbLoterias: JDBLoteria[]; genericLotteryConfigs: GenericLotteryConfig[]; casinoSettings: CasinoSettings; 
  bingoSettings: BingoSettings; bingoDraws: BingoDraw[]; bingoTickets: BingoTicket[]; bingoPayouts: BingoPayout[];
  snookerChannels: SnookerChannel[]; snookerPresence: Record<string, { viewers: string[] }>; snookerFinancialHistory: SnookerFinancialSummary[]; snookerBets: SnookerBet[]; snookerCashOutLog: any[]; snookerLiveConfig: SnookerLiveConfig; snookerActivityFeed: any[]; snookerBetsFeed: any[]; snookerChatMessages: SnookerChatMessage[]; snookerScoreboards: Record<string, SnookerScoreboard>; snookerSyncLogs: SnookerSyncLog[]; snookerAutomationSettings: SnookerAutomationSettings; celebrationTrigger: boolean;
  snookerSyncState: 'idle' | 'syncing' | 'success' | 'error'; snookerLastAutoCreatedIds: string[]; snookerLastAutoUpdatedIds: string[];
  ...
  syncSnookerFromYoutube: (force?: boolean) => Promise<void>; parseSnookerYoutubeItem: (item: any) => any; upsertAutoSnookerChannel: (channel: SnookerChannel) => void; approveAutoSnookerChannel: (id: string) => void; archiveAutoSnookerChannel: (id: string) => void; clearSnookerSyncLogs: () => void; updateSnookerAutomationSettings: (s: SnookerAutomationSettings) => void;
...
const DEFAULT_SNOOKER_CFG: SnookerLiveConfig = { defaultChannelId: '', showLiveBadge: true, betsEnabled: true, minBet: 5, maxBet: 1000, cashOutMargin: 8, chatEnabled: true, reactionsEnabled: true, profanityFilterEnabled: true, updatedAt: new Date().toISOString() };
...
const DEFAULT_SNOOKER_AUTOMATION: SnookerAutomationSettings = { enabled: true, sourceMode: 'hybrid', youtubeChannelUrl: 'https://www.youtube.com/@TVSnookerBrasil', autoCreateChannels: true, autoUpdateChannels: true, autoMarkLive: true, autoMarkFinished: true, autoFillPlayers: true, autoFillTournament: true, autoFillSchedule: true, autoFillPrize: true, syncIntervalSeconds: 300, dedupeWindowHours: 24, requireAdminApproval: false, overwriteManualFields: false, createScoreboardIfMissing: true, createDefaultOddsIfMissing: true, keepManualOdds: true, enabledOnHome: true, enabledOnPublicPage: true, allowPreMatchBetting: true, allowLiveBetting: true, defaultPreMatchBettingWindowMinutes: 60, autoCloseBettingOnStart: false };
...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [snookerLastAutoCreatedIds, setSnookerLastAutoCreatedIds] = useState<string[]>([]);
  const [snookerLastAutoUpdatedIds, setSnookerLastAutoUpdatedIds] = useState<string[]>([]);
  ...
  const syncSnookerFromYoutube = useCallback(async (force = false) => {
    ...
    const startLog: SnookerSyncLog = { id: `log-start-${Date.now()}`, createdAt: new Date().toISOString(), type: 'YOUTUBE_SYNC_START', status: 'info', message: 'Iniciando ciclo de sincronização...' };
    const logs = getStorageItem<SnookerSyncLog[]>('app:snooker_sync_logs:v1', []);
    setStorageItem('app:snooker_sync_logs:v1', [startLog, ...logs].slice(0, 100));

    try {
      const current = getStorageItem<SnookerChannel[]>('app:snooker_channels:v1', []);
      const { updatedChannels, summary } = await SnookerSyncService.sync(current, user?.bancaId || 'default', snookerAutomationSettings);
      
      // Auto-create scoreboards for new items
      ...
      setStorageItem('app:snooker_scores:v1', currentScores);

      const finishLog: SnookerSyncLog = { id: `log-fin-${Date.now()}`, createdAt: new Date().toISOString(), type: 'YOUTUBE_SYNC_FINISH', status: 'success', message: `Sync Finalizado: Lidos: ${summary.itemsRead} | Criados: ${summary.created} | Atualizados: ${summary.updated}`, payload: summary };
      ...
      notify();
      if (force) toast({ title: "Sincronização Concluída", description: `${summary.created} novos canais, ${summary.updated} atualizados.` });
    } catch (e: any) {
      setSnookerSyncState('error');
      if (force) toast({ variant: 'destructive', title: "Erro na sincronização", description: e.message });
    }
  }, [user, snookerAutomationSettings, notify, toast, snookerSyncState]);
  ...
  const clearSnookerSyncLogs = useCallback(() => { setStorageItem('app:snooker_sync_logs:v1', []); notify(); }, [notify]);
  const parseSnookerYoutubeItem = useCallback((item: any) => { return SnookerParserService.parse(item.snippet.title, item.snippet.description); }, []);
  const upsertAutoSnookerChannel = useCallback((channel: SnookerChannel) => { const current = getStorageItem<SnookerChannel[]>('app:snooker_channels:v1', []); const exists = current.find(c => c.id === channel.id || c.sourceVideoId === channel.sourceVideoId); if (exists) { setStorageItem('app:snooker_channels:v1', current.map(c => c.id === (exists.id) ? { ...c, ...channel } : c)); } else { setStorageItem('app:snooker_channels:v1', [channel, ...current]); } notify(); }, [notify]);
  const updateSnookerAutomationSettings = useCallback((s: SnookerAutomationSettings) => { setStorageItem('app:snooker_automation:v1', s); notify(); }, [notify]);
  ...
  const drawBingoBall = useCallback((id: string) => { const draws = getStorageItem<BingoDraw[]>('app:bingo_draws:v1', []); const idx = draws.findIndex(d => d.id === id); if (idx === -1) return; const draw = draws[idx]; const available = Array.from({ length: 90 }, (_, i) => i + 1).filter(n => !draw.drawnNumbers.includes(n)); if (available.length === 0) return; const ball = available[Math.floor(Math.random() * available.length)]; const newDrawn = [...draw.drawnNumbers, ball]; const tickets = getStorageItem<BingoTicket[]>('app:bingo_tickets:v1', []); const drawTickets = tickets.filter(t => t.drawId === id && t.status === 'active'); const winners = { ...draw.winnersFound }; drawTickets.forEach(t => { const hits = t.ticketNumbers.filter(n => newDrawn.includes(n)).length; if (!winners.quadra && hits >= 4) winners.quadra = { category: 'quadra', userId: t.userId, userName: t.userName, terminalId: t.terminalId, winAmount: draw.prizeRules.quadra, winningNumbers: t.ticketNumbers, wonAt: new Date().toISOString(), type: t.isBot ? 'BOT_WIN' : 'USER_WIN' }; if (!winners.kina && hits >= 5) winners.kina = { category: 'kina', userId: t.userId, userName: t.userName, terminalId: t.terminalId, winAmount: draw.prizeRules.kina, winningNumbers: t.ticketNumbers, wonAt: new Date().toISOString(), type: t.isBot ? 'BOT_WIN' : 'USER_WIN' }; if (!winners.keno && hits >= 15) winners.keno = { category: 'keno', userId: t.userId, userName: t.userName, terminalId: t.terminalId, winAmount: draw.prizeRules.keno, winningNumbers: t.ticketNumbers, wonAt: new Date().toISOString(), type: t.isBot ? 'BOT_WIN' : 'USER_WIN' }; }); draws[idx] = { ...draw, drawnNumbers: newDrawn, winnersFound: winners }; setStorageItem('app:bingo_draws:v1', draws); notify(); }, [notify]);
  ...
  const placeSnookerBet = useCallback((bet: any) => { if (!user) { toast({ variant: 'destructive', title: 'Login necessário' }); return false; } const result = BetService.processBet(user, { userId: user.id, modulo: 'Sinuca', valor: bet.amount, retornoPotencial: 0, descricao: `Sinuca: Aposta em ${bet.pick}`, referenceId: `sno-${bet.channelId}-${Date.now()}` }); if (result.success) { const newBet: SnookerBet = { ...bet, id: `bet-sno-${Date.now()}`, userId: user.id, userName: user.nome || user.terminal, status: 'open', createdAt: new Date().toISOString() }; const currentBets = getStorageItem<SnookerBet[]>('app:snooker_bets:v1', []); setStorageItem('app:snooker_bets:v1', [newBet, ...currentBets]); const feedItem = { id: `feed-${Date.now()}`, channelId: bet.channelId, userId: user.id, userName: user.nome || user.terminal, text: `apostou R$ ${bet.amount.toFixed(2)} em ${bet.pick}`, createdAt: new Date().toISOString() }; const currentFeed = getStorageItem<any[]>('app:snooker_bets_feed:v1', []); setStorageItem('app:snooker_bets_feed:v1', [feedItem, ...currentFeed].slice(0, 50)); notify(); return true; } return false; }, [user, notify, toast]);
  const cashOutSnookerBet = useCallback((betId: string) => { if (!user) return; const currentBets = getStorageItem<SnookerBet[]>('app:snooker_bets:v1', []); const bet = currentBets.find(b => b.id === betId); if (!bet || bet.status !== 'open') return; const currentChannels = getStorageItem<SnookerChannel[]>('app:snooker_channels:v1', []); const channel = currentChannels.find(c => c.id === bet.channelId); if (!channel) return; const oddsMap = { A: channel.odds.A, B: channel.odds.B, EMPATE: channel.odds.D }; const currentOdd = oddsMap[bet.pick]; const originalOdd = { A: bet.oddsA, B: bet.oddsB, EMPATE: bet.oddsD }[bet.pick]; const potentialPayout = bet.amount * originalOdd; const fairValue = potentialPayout / currentOdd; const margin = (snookerLiveConfig.cashOutMargin || 8) / 100; const cashOutValue = Math.max(0.01, fairValue * (1 - margin)); const u = getUserByTerminal(user.terminal); if (u) { const newBal = u.saldo + cashOutValue; upsertUser({ terminal: u.terminal, saldo: newBal }); LedgerService.addEntry({ bancaId: u.bancaId || 'default', userId: u.id, terminal: u.terminal, tipoUsuario: u.tipoUsuario, modulo: 'Sinuca', type: 'CASH_OUT_RECOLHE', amount: cashOutValue, balanceBefore: u.saldo, balanceAfter: newBal, referenceId: betId, description: `Cash Out Sinuca: ${bet.pick}` }); } const logEntry = { id: `co-${Date.now()}`, userId: user.id, userName: user.nome || user.terminal, channelId: bet.channelId, channelTitle: channel.title, originalAmount: bet.amount, originalOdd, cashOutOdd: currentOdd, cashOutAmount: cashOutValue, houseProfit: bet.amount - cashOutValue, createdAt: new Date().toISOString() }; const currentLogs = getStorageItem<any[]>('app:snooker_cashout:v1', []); setStorageItem('app:snooker_cashout:v1', [logEntry, ...currentLogs]); setStorageItem('app:snooker_bets:v1', currentBets.map(b => b.id === betId ? { ...b, status: 'cash_out' as const } : b)); toast({ title: 'Cash Out Concluído' }); notify(); }, [user, snookerLiveConfig, notify, toast]);
  const sendSnookerChatMessage = useCallback((channelId: string, text: string) => { if (!user) return; let filteredText = snookerLiveConfig.profanityFilterEnabled ? filterProfanity(text) : text; const newMessage: SnookerChatMessage = { id: `msg-${Date.now()}`, channelId, userId: user.id, userName: user.nome || user.terminal, text: filteredText, createdAt: new Date().toISOString(), role: ['ADMIN', 'SUPER_ADMIN'].includes(user.tipoUsuario) ? 'admin' : 'user' }; const currentMsgs = getStorageItem<SnookerChatMessage[]>('app:snooker_chat:v1', []); setStorageItem('app:snooker_chat:v1', [newMessage, ...currentMsgs].slice(0, 100)); notify(); }, [user, snookerLiveConfig, notify]);
  const deleteSnookerChatMessage = useCallback((id: string) => { const current = getStorageItem<SnookerChatMessage[]>('app:snooker_chat:v1', []); setStorageItem('app:snooker_chat:v1', current.map(m => m.id === id ? { ...m, deleted: true } : m)); notify(); }, [notify]);
  ...
  const updateSnookerScoreboard = useCallback((id: string, s: any) => { const current = getStorageItem<Record<string, SnookerScoreboard>>('app:snooker_scores:v1', {}); setStorageItem('app:snooker_scores:v1', { ...current, [id]: s }); notify(); }, [notify]);
  const addSnookerChannel = useCallback((c: any) => { const current = getStorageItem<SnookerChannel[]>('app:snooker_channels:v1', []); const newId = `chan-${Date.now()}`; setStorageItem('app:snooker_channels:v1', [...current, { ...c, id: newId, scoreA: 0, scoreB: 0, status: 'scheduled', enabled: true, odds: { A: 1.95, B: 1.95, D: 3.20 } }]); const currentScores = getStorageItem<Record<string, SnookerScoreboard>>('app:snooker_scores:v1', {}); currentScores[newId] = { matchTitle: c.title, playerA: { name: c.playerA.name, score: 0 }, playerB: { name: c.playerB.name, score: 0 }, scoreA: 0, scoreB: 0, statusText: 'Aguardando início', frame: 0, round: { number: 1 } }; setStorageItem('app:snooker_scores:v1', currentScores); notify(); }, [notify]);
  ...
  const contextValue = useMemo(() => ({
    ...
    snookerChannels, snookerPresence, snookerFinancialHistory, snookerBets, snookerCashOutLog, snookerLiveConfig, snookerActivityFeed, snookerBetsFeed, snookerChatMessages, snookerScoreboards, snookerSyncLogs, snookerAutomationSettings, celebrationTrigger, snookerSyncState, snookerLastAutoCreatedIds, snookerLastAutoUpdatedIds,
    joinChannel, leaveChannel, placeSnookerBet, cashOutSnookerBet, sendSnookerChatMessage, deleteSnookerChatMessage, sendSnookerReaction, updateSnookerLiveConfig, updateSnookerScoreboard, addSnookerChannel, updateSnookerChannel, deleteSnookerChannel, settleSnookerRound, clearCelebration, syncSnookerFromYoutube, parseSnookerYoutubeItem, upsertAutoSnookerChannel, approveAutoSnookerChannel, archiveAutoSnookerChannel, clearSnookerSyncLogs, updateSnookerAutomationSettings,
    ...
    user, allUsers, isLoading, ledger, banners, popups, news, apostas, postedResults, jdbResults, jdbSyncLogs, jdbLoterias, genericLotteryConfigs, footballData, footballBets, betSlip, liveMiniPlayerConfig, isFullscreen, bingoSettings, bingoDraws, bingoTickets, bingoPayouts, snookerChannels, snookerPresence, snookerFinancialHistory, snookerBets, snookerCashOutLog, snookerLiveConfig, snookerActivityFeed, snookerBetsFeed, snookerChatMessages, snookerScoreboards, snookerSyncLogs, snookerAutomationSettings, celebrationTrigger, snookerSyncState, snookerLastAutoCreatedIds, snookerLastAutoUpdatedIds, loadLocalData, logout, handleFinalizarAposta, processarResultados, syncFootballAll, updateLeagueConfig, placeFootballBet, addBanner, updateBanner, deleteBanner, addPopup, updatePopup, deletePopup, addNews, updateNews, deleteNews, toggleFullscreen, drawBingoBall, casinoSettings, updateCasinoSettings, publishJDBResult, deleteJDBResult, updateBingoSettings, createBingoDraw, startBingoDraw, finishBingoDraw, cancelBingoDraw, buyBingoTickets, refundBingoTicket, payBingoPayout, registerCambistaMovement, joinChannel, leaveChannel, placeSnookerBet, cashOutSnookerBet, sendSnookerChatMessage, deleteSnookerChatMessage, sendSnookerReaction, updateSnookerLiveConfig, updateSnookerScoreboard, addSnookerChannel, updateSnookerChannel, deleteSnookerChannel, settleSnookerRound, clearCelebration, syncSnookerFromYoutube, parseSnookerYoutubeItem, upsertAutoSnookerChannel, approveAutoSnookerChannel, archiveAutoSnookerChannel, clearSnookerSyncLogs, updateSnookerAutomationSettings
  ]);
